...
export async function buildBasquioSystemPrompt(input: {
  ...
  const staticBlock = [
    ...
    "- For NielsenIQ promo work, SCQA is only the narrative wrapper. The body of the deck must drill down mechanically across market, channel, retailer/area, format, competitor, promo mechanic, and productivity. Do not replace that depth with polished synthesis.",
    "- CLAIM-TO-CHART BINDING: if the slide says the issue is rotation, productivity, ROS, price-led growth, or a distribution opportunity, the hero exhibit must show that metric or a direct causal driver of it. Side notes do not rescue a mismatched chart.",
    "- REDUNDANCY RULE: a new slide must introduce a genuinely new analytical question, deeper grain, different metric family, or different causal driver. Different commentary on the same data cut is not a new slide.",
    "- STORYLINE CONTIGUITY: keep each analytical branch contiguous. Do not move from segments to channels and then back to segments unless the revisit is an explicit comparison/synthesis or a clearly deeper follow-up.",
    "- FOCAL BRAND PERSISTENCE: on competitor, player, or manufacturer slides, explicitly keep the focal brand visible and interpret what the comparison means for that focal brand.",
    "- PROMO STORY CONTRACT: for promo-led briefs, move through category baseline, value vs volume vs price, promo vs no-promo, discount tiers, channel/format/localization, focal brand vs competitor, WD Promo/display/folder/communication-in-store mechanics, and only then a short synthesis.",
    "- PROMO MECHANIC COMPLETENESS: if the source workbook includes Communication In Store or Comm. In Store Only, include that mechanic in the promo-mechanics exhibit instead of showing only price-cut or display mechanics. If slide copy mentions WD Promo, DP promo, display, folder, or Communication In Store, the hero exhibit must show that same metric or mechanic directly.",
    "- DECIMAL POLICY: use deterministic NIQ metric formatting. Value/volume/packs use 0 decimals unless scaled, distribution/promo pressure/TDP use 0, shares and discount depth use 1, prices use 2, indices use 0 except intensity index which uses 1, and rotation/ROS/productivity use 1. Variations inherit the base precision.",
    ...
    "- NATIVE TABLE RULE: if a slide uses the table archetype or a supporting grid of exact numbers, build it with slide.addTable(), set hasDataTable: true in deck_manifest.json, and do not fake the table with separate text boxes.",
    ...
    "- For supported chart families, `data_tables.xlsx` should attempt to include a native XlsxWriter chart object linked to that same sheet.",
    ...
    "- Excel-native charts are best-effort companion artifacts. If XlsxWriter is unavailable or one chart object fails, still write the exact sheet, omit `excelChartCellAnchor` for that chart, and continue. Never let Excel chart generation block the PPTX, PDF, markdown report, or workbook.",
...
function buildHaikuReportOnlySystemPrompt(input: {
  ...
  return [
    ...
    "- Use Python and pandas as the default execution path. When writing `data_tables.xlsx` for chart-bearing sheets, use XlsxWriter so native Excel chart objects can be attempted when the runtime allows it.",
