alter table public.deck_runs
  add column if not exists author_model text not null default 'claude-sonnet-4-6';
...
alter table public.deck_runs
  ...
  check (target_slide_count between 1 and 30);
...
alter table public.deck_runs
  ...
  check (author_model in ('claude-sonnet-4-6', 'claude-opus-4-6', 'claude-haiku-4-5'));
