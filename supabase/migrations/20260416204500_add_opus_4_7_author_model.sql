alter table public.deck_runs
  drop constraint if exists deck_runs_author_model_check;

alter table public.deck_runs
  add constraint deck_runs_author_model_check
  check (author_model in ('claude-sonnet-4-6', 'claude-opus-4-7', 'claude-opus-4-6', 'claude-haiku-4-5'));

alter table public.recipes
  drop constraint if exists recipes_author_model_check;

alter table public.recipes
  add constraint recipes_author_model_check
  check (author_model in ('claude-sonnet-4-6', 'claude-opus-4-7', 'claude-opus-4-6', 'claude-haiku-4-5'));

alter table public.template_fee_checkout_drafts
  drop constraint if exists template_fee_checkout_drafts_author_model_check;

alter table public.template_fee_checkout_drafts
  add constraint template_fee_checkout_drafts_author_model_check
  check (author_model in ('claude-sonnet-4-6', 'claude-opus-4-7', 'claude-opus-4-6', 'claude-haiku-4-5'));
...
declare
  ...
  v_author_model text := case
    when p_author_model = 'claude-opus-4-6' then 'claude-opus-4-7'
    else p_author_model
  end;
begin
  ...
  if v_author_model not in ('claude-sonnet-4-6', 'claude-opus-4-7', 'claude-haiku-4-5') then
    raise exception 'author_model must be claude-sonnet-4-6, claude-opus-4-7, or claude-haiku-4-5';
  end if;
