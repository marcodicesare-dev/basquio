...
async function main() {
  ...
  const summary = buildAuthoringToolCallSummary(MODEL, { webFetchMode: options.webFetchMode });
  assertAuthoringExecutionContract({
    model: MODEL,
    phase: "smoke",
    tools,
  ...
  if (!codeExecutionStarted) {
    throw new Error(
      `Smoke failed: Claude response did not include any code execution blocks. stop_reason=${
