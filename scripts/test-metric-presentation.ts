...
import { inferMetricPresentationSpec } from "../packages/workflows/src/metric-presentation";
import { lintDeckFidelity } from "../packages/intelligence/src/fidelity-validators";

function main() {
  const salesValue = inferMetricPresentationSpec({ label: "Sales Value", title: "Category" });
  assert.equal(salesValue.decimalPlaces, 0);

  const salesValueMillions = inferMetricPresentationSpec({ label: "Sales Value (Mln)", title: "Category" });
  ...
  const price = inferMetricPresentationSpec({ label: "Avg Promo Price (LTRS)", title: "Promo" });
  assert.equal(price.decimalPlaces, 2);

  const avgRefs = inferMetricPresentationSpec({ label: "Numero medio di referenze", title: "Shelf" });
  ...
  const intensityIndex = inferMetricPresentationSpec({ label: "Intensity Index", title: "Promo" });
  ...
  process.stdout.write("metric presentation and claim-chart alignment regressions passed\n");
}
