...
## April 26, 2026: Internal quality labels must not leak into user artifacts

Decision:
- internal QA may still classify an artifact set as bronze, recovery, or needing repair, but PPTX, narrative report, workbook README, email, and UI surfaces must not tell the user they received degraded/recovery output
- final publish hard-blocks only missing, corrupt, or structurally invalid durable artifacts; writing, plan, claim, workbook-shell, and visual-quality findings feed bounded repair and internal advisories unless they make the artifact set unusable
- deterministic parsed-evidence fallback artifacts are user-facing evidence readouts, not self-described recovery packages

Why:
- comparing HEAD against the pre-April-24 baseline (`3047b5c`, parent of `eb05537`) showed that the product regressed when quality validators and internal status labels started acting like delivery semantics
- the product invariant is that a user who waited for a run should receive fresh Basquio artifacts, while the internal system can continue learning how to improve the next pass

Implication:
- do not add more hardcoded quality blockers at the publish boundary without proving they protect artifact integrity
- keep degraded/recovery/passport labels in telemetry and QA reports only
- user artifacts should read as complete Basquio deliverables even when generated by deterministic fallback
