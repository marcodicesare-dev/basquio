...
- A paid author phase that fails before producing a publishable artifact set must still reach deterministic recovery publish from parsed evidence. Do not let author validation throws bypass the artifact publisher unless the attempt lost ownership or the worker is shutting down.
- Merged full-deck author runs must attempt plan-quality repair before revise. Fabricated sheet names, duplicate analytical cuts, storyline backtracking, unsupported content counts, and blocking plan-depth failures trigger one bounded author rebuild of `analysis_result.json`, `deck.pptx`, `narrative_report.md`, `data_tables.xlsx`, and `deck_manifest.json`. If the rebuilt plan still fails, keep the best structurally publishable fresh artifact set with internal advisories; if no publishable author artifact set exists, fall through to deterministic recovery publish rather than spending unbounded revise budget or leaving the user with nothing.
- Publish gates must not turn internal QA into an empty user outcome. Final publish may hard-block missing, corrupt, or structurally invalid durable artifacts. Writing, plan, claim, workbook-shell, and visual-quality failures should drive bounded repair and internal advisories unless they leave the artifact set unusable.
- `qa_passed` and `delivery_status` must reflect whether durable artifacts are publishable, not whether the internal quality passport is gold, silver, bronze, or recovery.
- User-facing artifacts must not expose internal delivery states such as degraded, recovery, quality passport, or review-before-client-use. Internal telemetry can keep those labels, but PPTX, narrative report, workbook README, emails, and UI copy must read like a fresh Basquio delivery.
- `narrative_report.md` depth, required sections, and Italian orthography, plus `data_tables.xlsx` README, formatted tables, freeze panes, widths, sheet links, and native chart companions remain part of the quality contract. Narrative depth should scale by deck size so short executive decks do not burn the author turn on oversized prose before durable files exist.
