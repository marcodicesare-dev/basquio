...
- Publish gates must block weak durable artifacts. `narrative_report.md` depth, required sections, and Italian orthography, plus `data_tables.xlsx` README, formatted tables, freeze panes, widths, sheet links, and native chart companions are part of the user-facing quality contract. Narrative depth should scale by deck size so short executive decks do not burn the author turn on oversized prose before durable files exist.
