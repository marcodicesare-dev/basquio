...
- AI SDK v6 `LanguageModelUsage` exposes cache stats nested under `usage.inputTokenDetails` (`cacheReadTokens`, `cacheWriteTokens`, `noCacheTokens`), not at the top level. The previous Anthropic-style flat keys `cache_creation_input_tokens` / `cache_read_input_tokens` exist only on the raw Anthropic SDK response. Code that reads cache telemetry from a `streamText`/`generateText` result must read `usage.inputTokenDetails.cacheReadTokens` and `usage.inputTokenDetails.cacheWriteTokens`. Discovered when Memory v1 Brief 2 `__chat_turn__` rows were writing NULL cache columns on production despite the cache mechanism actively returning hits; fixed in commit `9132e1c`.
- `vercel env add NAME production` reads the value from stdin verbatim, INCLUDING any trailing newline. `echo "true" | vercel env add ...` saves the value as `"true\n"`. Use `printf "true" | vercel env add ...` (no trailing newline) or paste the value via the interactive prompt. The bug is silent on the writer side; downstream code reads `process.env.NAME === "true"` and gets false. Discovered during Memory v1 Brief 2 Phase 9 flag flip.
- Native-module dependencies in `packages/workflows` that are imported transitively from Next.js API routes (e.g. `@boundaryml/baml` from Brief 3 brand-extraction `process.ts` upload route) MUST be added to `apps/web/next.config.ts` `serverExternalPackages`. Webpack tries to bundle `.node` binaries by default and fails with "Unexpected character" parse errors on the binary content. The same pattern applies to any future native-binding dep (BAML's platform packages get hoisted automatically when the parent is externalized, same as `sharp` and `@resvg/resvg-js`). Local `pnpm tsc --noEmit` does NOT catch this because TypeScript skips Webpack bundling; only `pnpm --filter @basquio/web build` reproduces the Vercel build failure locally. Add this build smoke to the local gate for any brief that adds workflows-side native deps. Discovered during Memory v1 Brief 3 Phase 9 Vercel deploy after PUSH 1; fixed in Brief 3 PUSH 2 (commit on origin/main).
...
April 27, 2026 (continued, Brief 6 PUSH 1-3): Memory v1 Brief 6 admin console v1 shipped across three pushes. Migration `20260519100000_super_admins.sql` adds the `super_admins` table (Marco seeded with two user_ids: marcodicesare1992@gmail.com 52aa79f4-... and marco@basquio.com a2dd82d3-...) plus the `is_super_admin(_user_id)` SECURITY DEFINER STABLE helper. apps/web/src/app/admin/layout.tsx renders the 9-route sidebar shell + auth gate (unauthenticated -> /sign-in, non-admin -> Forbidden, admin -> shell). All 9 spec §10 routes shipped as read-only Next.js server components: /admin (overview, last 7d aggregates), /admin/runs + /admin/runs/[id] (chat-turn replay), /admin/audit (filtered memory_audit), /admin/candidates (memory_candidates with status counters), /admin/hints (anticipation_hints ledger), /admin/drift (repeated cooldown_key dismissals + stale candidates), /admin/cost (chat-cost aggregated by workspace 30d), /admin/prompts (hard-coded prompt registry), /admin/skills (filesystem listing of skills/ with fallback). All loaders live in apps/web/src/lib/admin/loaders.ts (service role; the layout already gated). `metadata.robots = no-index/no-follow`. With Brief 6 live, all six Memory v1 briefs are shipped behind feature flags. Forensic: `docs/research/2026-04-27-brief-6-shipped.md`.

April 27, 2026 (continued, Brief 5 PUSH 1-3): Memory v1 Brief 5 shipped across three pushes covering procedural rule injection (PART A, commit `3517b11`), Memory Inspector v2 4-tab UI (PART B, commit `ebf8561`, behind `MEMORY_INSPECTOR_V2`), and anticipation hints (PART C, commit `01a1886`, behind `ANTICIPATION_ENABLED`). Two migrations: `20260512100000_workspace_rule_rpcs.sql` adds 4 SECURITY DEFINER RPCs (upsert / pin / edit / forget workspace_rule); `20260512110000_anticipation_hints_rpcs.sql` adds 5 RPCs (insert / dismiss / snooze / accept / expire) with 14-day cooldown_key suppression enforced inside insert_anticipation_hint. PART A wires `buildScopeContextPack` to inject active workspace_rule rows ordered by priority desc into every chat scope pack (capped at 24 rules). PART B's `/workspace/memory` page renders 4 tabs (Entities, Facts, Rules, Pending) when MEMORY_INSPECTOR_V2=true; flag-OFF falls back to the legacy MemoryBrowser. PART C's `WorkspaceHintsBanner` renders 3-cap hints on the workspace home with Done / Snooze 7d / Dismiss actions. Inngest stays retired (same finding as Briefs 3-4); pg_cron schedules for nightly expiry are guarded by IF EXISTS pg_extension. pg_cron is currently NOT enabled on the basquio project; operator can enable via Dashboard -> Database -> Extensions when desired. The three Monday-morning hint generators are pull-based (pending candidates older than 3 days, brand_guideline extracted in last 7 days, workspace_rule activity in last 14 days) and callable from scripts or the upcoming Brief 6 admin console; full pg_cron-driven automation deferred to a future brief once production observation calibrates the signal-to-noise ratio. Local gates 304/304. Forensic: `docs/research/2026-04-27-brief-5-shipped.md`.

April 27, 2026 (continued, Brief 4 PUSH 1): Memory v1 Brief 4 chat-turn fact extractor + memory_candidates queue shipped behind `CHAT_EXTRACTOR_ENABLED` flag (default false, DRY MODE). Two migrations applied: `20260505100000_memory_candidates.sql` (table + RLS + 3 indices including `idx_memory_candidates_workspace_pending` partial on status='pending'), `20260505110000_memory_candidates_rpcs.sql` (5 SECURITY DEFINER RPCs: insert_memory_candidate, approve_memory_candidate, dismiss_memory_candidate, expire_pending_candidates, auto_promote_high_confidence, plus internal helper write_durable_memory_from_candidate; pg_cron schedule guarded by `IF EXISTS pg_extension`). Mem0 V3-style ADD-only extractor at `packages/workflows/src/workspace/chat-extraction.ts` runs Haiku 4.5 with `structuredOutputMode: 'jsonTool'` post-streaming via Next.js `after()` from "next/server" (Inngest-retired-on-basquio architecture, same pattern as Brief 3 Option C). Confidence gates: < 0.6 dropped, 0.6-0.8 inserted as pending, > 0.8 auto-promoted ONLY when flag is true (DRY MODE keeps everything pending). Auto-promote dispatches by kind into facts (kind='fact'), workspace_rule (kind='rule'), memory_entries (kind='preference'); kinds 'alias' and 'entity' stage in memory_entries with `metadata.deferred_kind` until Brief 5 wires entity-resolution. Local gates 295/295. 40-turn live eval against the labeled fixture (`apps/web/src/lib/workspace/__tests__/fixtures/chat-extraction-eval.json`) returned auto-promote precision 96.2% (PASS, target >= 95%), false-positive rate 0.59/10 (1 borderline FP on a possessive-brand turn), 100% kind coverage, $0.093 total cost. Phase 8 (24-48h dry-mode observation on real team-beta traffic) is the load-bearing gate, not the 40-turn fixture; PUSH 3 flips the flag after the observation passes, PUSH 4 reserved for prompt tuning with real-distribution data if it does not. Candidate-queue UI placeholder wired into the existing `/workspace/memory` page above the MemoryBrowser. Forensic: `docs/research/2026-04-27-brief-4-shipped.md`, `docs/research/2026-04-27-brief-3-shipped.md` (canonical SECURITY DEFINER RPC reference).

April 27, 2026 (continued, Brief 3 PUSH 1): Memory v1 Brief 3 brand-guideline extraction shipped behind `BRAND_EXTRACTION_ENABLED` flag (default false). Two migrations applied: `20260428140000_brand_extraction_rpc.sql` (SECURITY DEFINER `public.persist_brand_guideline` setting `app.actor` and `app.workflow_run_id` inside the function body, the canonical pattern Briefs 4-6 reuse), `20260428141000_knowledge_documents_brand_book_kind.sql` (widens kind CHECK to include `brand_book`). BAML 0.221.0 added at workspace root; sources at `packages/workflows/baml_src/` (clients.baml + brand_guideline.baml), generated client at `packages/workflows/baml_client/` (gitignored, regenerated by `pnpm baml:gen` from postinstall and `qa:basquio`). Three-phase pipeline at `packages/workflows/src/workspace/brand-extraction.ts`: Sonnet 4.6 extract → Haiku 4.5 validate (reject below 0.7) → SECURITY DEFINER persist. Wired into `processWorkspaceDocument` post-ingest hook (Option C: extends the existing Railway worker rather than reviving the retired Inngest endpoint). Upload route + UI add a `kind: 'brand_book'` toggle on `WorkspaceUploadZone`. queryBrandRuleTool refactored to call `getActiveBrandGuideline` from new `apps/web/src/lib/workspace/brand-guidelines.ts` (also exports `searchBrandRules` for Brief 5). Local gates 281/281 (10 new tests: 4 brand-extraction + 6 brand-guidelines). Live extraction smoke against `fixtures/brand-books/spotify.pdf` (21 pages, 11k chars text-extracted) returned 0 typography / 5 colour / 3 tone / 3 imagery rules with 100% source_page coverage on every extracted rule, validation confidence 0.68 (correctly below the 0.7 persist floor, sparse extraction rejected), negative test confidence 0.15 catching all 12 hard-fail violations, total cost $0.04. Phase 9 (flag flip + production verification) deferred until a text-rich brand book is available. Forensic: `docs/research/2026-04-27-brief-3-shipped.md`, `docs/research/2026-04-27-brief-3-substrate-audit.md`.

- BAML brand-extraction acceptance gates from spec §4 (≥5 tone rules, ≥8 typography rules, ≥12 colour entries) presuppose a 100+ page text-rich brand book (Kempinski class). Image-heavy or short brand PDFs (e.g. Spotify 21pg with visual typography mockups that pdf-parse cannot OCR) trigger sparse extraction; the validator correctly rejects them at confidence < 0.7 and the pipeline declines to persist. Production verification of the rule-count gates requires a text-rich CPG-style brand book (Mondelez, Mulino Bianco, Lavazza, BBC Brand Guidelines, NHS Identity Guidelines, Kempinski). Spotify-class fixtures stay valid for negative-test and source-page coverage smokes only. If multiple text-rich production brand books also produce sparse extractions, the fix is multimodal PDF input (BAML `pdf` type instead of `pdf_text` string; Claude reads visual pages natively), not prompt tuning. PUSH 4 of the Brief 3 budget is reserved for that migration if Phase 9 surfaces it.

- BAML extraction prompts must avoid literal `{` and `}` in the prompt body. Jinja templating consumes `{{ ... }}` for interpolation, and embedded JSON examples like `{"a": "b"}` cause `baml-cli generate` to silently misparse the prompt and report a syntax error on the next valid keyword (e.g. `function ExtractFoo` flagged as "not a known schema keyword" because the parser thinks it is still inside the prompt body). Either describe shapes in plain text or use a typed BAML class field (e.g. `LanguagePreference[]` instead of `map<string, string>` with embedded JSON examples in the prompt). Discovered while writing `packages/workflows/baml_src/brand_guideline.baml` for Brief 3.

April 27, 2026 (continued, Phase 9): Memory v1 Brief 2 flag CHAT_ROUTER_V2_ENABLED flipped to true on Vercel production (basquio-web project) at 14:01 UTC. Initial flip captured a stray newline (`echo "true"` artifact); corrected via `printf` and a second redeploy. Final activation on commit `9132e1c` after a second fix landed for the AI SDK `usage.inputTokenDetails` nesting (`__chat_turn__` rows were writing NULL cache columns). Live 5-turn smoke against basquio.com confirmed end-to-end: 5/5 `__chat_turn__` rows with intents/active_tools/classifier output populated, cache reads 6,227-6,612 tokens on every turn, costs $0.025-$0.034 on retrieval turns and $0.13 on the webSearch turn, intent gating correct (queryEntityFact for graph, queryBrandRule for rule, webSearch for web). Three temp basquio.com smoke users (`brief2-smoke-*@basquio.com`) remain in auth.users due to a residual Supabase Auth deletion trigger that the service role can't bypass; they have no associated workspace data and are inert.
