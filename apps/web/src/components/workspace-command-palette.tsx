...
/**
 * Global Cmd+K command palette mounted in the workspace shell so every
 * route (home, memory, sources, people, scopes, chats) has the same
 * navigation surface. Actions are built server-side in the workspace
 * layout from the same data that feeds the sidebar.
 *
 * Pattern: Linear, Notion, Cursor, and Vercel AI Chatbot all ship one
 * palette mounted globally. Mounting per-route (the previous
 * scope-only palette) leaves home/memory/sources/people without a
 * keyboard navigation surface, which is what Marco called out.
 *
 * The `triggerLabel` prop controls the visible button copy. Pass an
 * empty string to hide the trigger and keep the palette
 * keyboard-only; default renders a small "Search" pill that doubles as
...
export function WorkspaceCommandPalette({
  ...
  placeholder = "Search home, memory, sources, people, scopes, recent chats",
}: {
    ...
    <>
      {triggerLabel ? (
        <button
          ref={triggerRef}
          type="button"
          className="wbeta-command-trigger"
        ...
        >
          ...
          <span className="wbeta-command-trigger-label">{triggerLabel}</span>
          <kbd className="wbeta-command-trigger-kbd">⌘K</kbd>
        </button>
      ) : null}
            ...
            <div className="wbeta-command-search">
              ...
              <input
                ...
                aria-activedescendant={
                  activeAction ? `workspace-command-${activeAction.id}` : undefined
                }
              />
              <button type="button" onClick={closePalette} aria-label="Close search">
                <X size={14} weight="bold" />
              </button>
            </div>
            <ul className="wbeta-command-list" id="workspace-command-results">
              {filtered.map((action, index) => (
                <li key={action.id}>
                  <Link
                    href={action.href}
                    id={`workspace-command-${action.id}`}
                    className={
                      index === activeIndex
